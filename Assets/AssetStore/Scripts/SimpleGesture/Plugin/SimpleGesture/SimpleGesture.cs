...
// +-------------------------------------------------------------------------------------------------------------------+
// | SIMPLE GESTURE RECOGNIZER v.2.1:                                                                                  |
...
public partial class SimpleGesture : MonoBehaviour {
    ...
    private void Update() {
        ...
        if (Input.touchCount == 2) {
            ...
            if (!this.touches.ContainsKey(tID1))
                return;
            if (!this.touches.ContainsKey(tID2))
                return;

            TouchPhase phase = TouchPhase.Stationary;
            if (phase1 == TouchPhase.Moved || phase2 == TouchPhase.Moved)
                phase = TouchPhase.Moved;
            if (phase1 == TouchPhase.Began || phase2 == TouchPhase.Began)
                phase = TouchPhase.Began;
            if (phase1 == TouchPhase.Ended || phase2 == TouchPhase.Ended)
                phase = TouchPhase.Ended;
            if (phase1 == TouchPhase.Canceled || phase2 == TouchPhase.Canceled)
