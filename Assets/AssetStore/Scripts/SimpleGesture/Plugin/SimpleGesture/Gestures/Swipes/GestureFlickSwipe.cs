...
public class GestureFlickSwipe : BaseGesture {
    ...
    public override void AddDelegate(SimpleGesture.GestureDelegate method) {
        if (!this.tmp_orth) {
            switch (this.tmp_type) {
                case SimpleGesture.E_DIRECTION.ANY:
                    this.broadcastOn9AxisSwipeANY += method;
                    break;
                case SimpleGesture.E_DIRECTION.LFT:
                    this.broadcastOn9AxisSwipeLFT += method;
                    break;
                case SimpleGesture.E_DIRECTION.RHT:
                    this.broadcastOn9AxisSwipeRHT += method;
                    break;
                case SimpleGesture.E_DIRECTION.UPR:
                    this.broadcastOn9AxisSwipeUPR += method;
                    break;
                case SimpleGesture.E_DIRECTION.DWN:
                    this.broadcastOn9AxisSwipeDWN += method;
                    break;
                case SimpleGesture.E_DIRECTION.DDR:
                    this.broadcastOn9AxisSwipeDDR += method;
                    break;
                case SimpleGesture.E_DIRECTION.DUR:
                    this.broadcastOn9AxisSwipeDUR += method;
                    break;
                case SimpleGesture.E_DIRECTION.DDL:
                    this.broadcastOn9AxisSwipeDDL += method;
                    break;
                case SimpleGesture.E_DIRECTION.DUL:
                    this.broadcastOn9AxisSwipeDUL += method;
                    break;
                default:
                    Debug.LogError(SimpleGesture.ERROR_ENUM_DOES_NOT_CORRESPOND + " " + this.tmp_orth + " " + this.tmp_type);
                    break;
            }
        }
        else {
            switch (this.tmp_type) {
                case SimpleGesture.E_DIRECTION.LFT:
                    this.broadcastOn4AxisSwipeLFT += method;
                    break;
                case SimpleGesture.E_DIRECTION.RHT:
                    this.broadcastOn4AxisSwipeRHT += method;
                    break;
                case SimpleGesture.E_DIRECTION.UPR:
                    this.broadcastOn4AxisSwipeUPR += method;
                    break;
                case SimpleGesture.E_DIRECTION.DWN:
                    this.broadcastOn4AxisSwipeDWN += method;
                    break;
                default:
                    Debug.LogError(SimpleGesture.ERROR_ENUM_DOES_NOT_CORRESPOND + " " + this.tmp_orth + " " + this.tmp_type);
                    break;
    ...
    public void AddDelegate(SimpleGesture.GestureDelegate<GestureInfoSwipe> method) {
        if (!this.tmp_orth) {
            switch (this.tmp_type) {
                case SimpleGesture.E_DIRECTION.ANY:
                    this.broadcastOn9AxisSwipeANYI += method;
                    break;
                case SimpleGesture.E_DIRECTION.LFT:
                    this.broadcastOn9AxisSwipeLFTI += method;
                    break;
                case SimpleGesture.E_DIRECTION.RHT:
                    this.broadcastOn9AxisSwipeRHTI += method;
                    break;
                case SimpleGesture.E_DIRECTION.UPR:
                    this.broadcastOn9AxisSwipeUPRI += method;
                    break;
                case SimpleGesture.E_DIRECTION.DWN:
                    this.broadcastOn9AxisSwipeDWNI += method;
                    break;
                case SimpleGesture.E_DIRECTION.DDR:
                    this.broadcastOn9AxisSwipeDDRI += method;
                    break;
                case SimpleGesture.E_DIRECTION.DUR:
                    this.broadcastOn9AxisSwipeDURI += method;
                    break;
                case SimpleGesture.E_DIRECTION.DDL:
                    this.broadcastOn9AxisSwipeDDLI += method;
                    break;
                case SimpleGesture.E_DIRECTION.DUL:
                    this.broadcastOn9AxisSwipeDULI += method;
                    break;
                default:
                    Debug.LogError(SimpleGesture.ERROR_ENUM_DOES_NOT_CORRESPOND + " " + this.tmp_orth + " " + this.tmp_type);
                    break;
            ...
            switch (this.tmp_type) {
                case SimpleGesture.E_DIRECTION.LFT:
                    this.broadcastOn4AxisSwipeLFTI += method;
                    break;
                case SimpleGesture.E_DIRECTION.RHT:
                    this.broadcastOn4AxisSwipeRHTI += method;
                    break;
                case SimpleGesture.E_DIRECTION.UPR:
                    this.broadcastOn4AxisSwipeUPRI += method;
                    break;
                case SimpleGesture.E_DIRECTION.DWN:
                    this.broadcastOn4AxisSwipeDWNI += method;
                    break;
                default:
                    Debug.LogError(SimpleGesture.ERROR_ENUM_DOES_NOT_CORRESPOND + " " + this.tmp_orth + " " + this.tmp_type);
                    break;
    ...
    private bool IsSwipe(TouchInfo touchInfo) {
        ...
        if (curDistance < SWIPE_MIN_DISTANCE)
            return false;
        if (curDistance - (SWIPE_ERR_DISTANCE * minDistance) > minDistance)
            return false;
        return true;
    }
    ...
    private SimpleGesture.GestureDelegate Get9AxisDelegate(TouchInfo touchInfo) {
        ...
        if (angle <= 20.0f) {
            ...
            if (direction.y > 0.0f)
                return this.broadcastOn9AxisSwipeDUR;
            else
                return this.broadcastOn9AxisSwipeDDR;
        }
        else if (angle > 70.0f && angle <= 110.0f) {
            if (direction.y > 0.0f)
                return this.broadcastOn9AxisSwipeUPR;
            else
                return this.broadcastOn9AxisSwipeDWN;
        }
        else if (angle > 110.0f && angle < 160.0f) {
            if (direction.y > 0.0f)
                return this.broadcastOn9AxisSwipeDUL;
            else
                return this.broadcastOn9AxisSwipeDDL;
        }
