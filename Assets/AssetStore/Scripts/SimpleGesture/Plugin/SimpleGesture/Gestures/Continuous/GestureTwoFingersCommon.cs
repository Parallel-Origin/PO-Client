...
public class GestureTwoFingersCommon : BaseGesture {
    ...
    public override void AddDelegate(SimpleGesture.GestureDelegate method) {
        switch (this.tmp_type) {
            case SimpleGesture.E_2FINGERSCOMMON.PANNING:
                this.broadcastWhilePanning += method;
                break;
            case SimpleGesture.E_2FINGERSCOMMON.PINCHING:
                this.broadcastWhilePinching += method;
                break;
            case SimpleGesture.E_2FINGERSCOMMON.STRETCHING:
                this.broadcastWhileStretching += method;
                break;
            case SimpleGesture.E_2FINGERSCOMMON.TWISTING:
                this.broadcastWhileTwisting += method;
                break;
        }
    }
    ...
    public void AddDelegate(SimpleGesture.GestureDelegate<GestureInfoZoom> method) {
        switch (this.tmp_type) {
            case SimpleGesture.E_2FINGERSCOMMON.PINCHING:
                this.broadcastWhilePinchingI += method;
                break;
            case SimpleGesture.E_2FINGERSCOMMON.STRETCHING:
                this.broadcastWhileStretchingI += method;
                break;
        }
    }
    ...
    private bool IsPinching(float angle1, float angle2) {
        if (angle1 > 160.0f && angle2 > 160.0f)
            return true;
        return false;
    }

    private bool IsStretching(float angle1, float angle2) {
        if (angle1 < 20.0f && angle2 < 20.0f)
            return true;
        return false;
    }

    private bool IsTwisting(float angle1, float angle2) {
        if (angle1 > 60.0f && angle1 < 120.0f && angle2 > 60.0f && angle2 < 120.0f)
            return true;
        return false;
    }
