...
namespace Script.Client.Prototypers {
    /// <summary>
    ///     A <see cref="Prototyper{I,T}" /> for cloning and instantiating <see cref="Entity" /> which represents a structure in the game.
    /// </summary>
    public class StructurePrototyper : Prototyper<Entity> {
        ...
        public override void Initialize() {
            ...
            var defaultStructure = _entityManager.CreateArchetype(required.ToArray());

            // The default structure
            Register(1, () => _entityManager.CreateEntity(defaultStructure), (ref Entity instance) => {});
        }

        public override void AfterInstanced(short typeID, ref Entity clonedInstance) {
            base.AfterInstanced(typeID, ref clonedInstance);
            
            _entityManager.SetComponentData(clonedInstance, new Identity {ID = 0, Type = "4:" + typeID});
