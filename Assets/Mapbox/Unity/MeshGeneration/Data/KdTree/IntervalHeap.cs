    ...
    public class IntervalHeap<T> {
        ...
        private void SiftInsertedValueUp() {
            ...
            // If it is on the max side, 
                ...
                if (tKeys[u] < tKeys[u - 1]) { // If less than it's pair
                    u = Swap(u, u - 1); // Swap with it's pair
                    if (tKeys[u] < tKeys[p - 1]) { // If smaller than smaller parent pair
                    ...
                    if (tKeys[u] > tKeys[p]) { // If larger that larger parent pair
                ...
                else if (tKeys[u] < tKeys[p - 1]) { // If smaller than smaller parent pair
                    // Swap into min-heap side
                    u = Swap(u, p - 1);
                    SiftUpMin(u);
                }
