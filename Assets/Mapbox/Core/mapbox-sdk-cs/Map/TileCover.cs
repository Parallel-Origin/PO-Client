...
namespace Mapbox.Map {
    ...
    public static class TileCover {
        ...
        public static Vector2d TileIdToCoordinate(UnwrappedTileId tileId) {


            // See: http://wiki.openstreetmap.org/wiki/Slippy_map_tilenames

            double n = Math.PI - ((2.0 * Math.PI * tileId.Y) / Math.Pow(2.0, tileId.Z));

            var lat_deg = (double)((tileId.X / Math.Pow(2.0, tileId.Z) * 360.0) - 180.0);
            var lon_deg = (double)(180.0 / Math.PI * Math.Atan(Math.Sinh(n)));

            return new Vector2d(lat_deg, lon_deg);
